...
public class BaseHtml implements LoadCondition {
            ...
            public void run() {
                if (getDoc() == null && !isIni()) {
                    try {
                        ...
                        String userAgentArray = userAgentArrays[random.nextInt(userAgentArrays.length)];
                        connection.userAgent(userAgentArray);
                        connection.ignoreContentType(true);
                        connection.ignoreHttpErrors(true);
                        connection.followRedirects(false);
                        SSLSocketFactory sslFactory = new SSLSocketFactoryImpl();
                        connection.sslSocketFactory(sslFactory);
                        Document docValue = connection.get();

                        setDoc(docValue);
                        setHost(new URL(docValue.baseUri()).getHost());
                        setIni(true);
                    } catch (IOException e) {
