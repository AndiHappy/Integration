 ...
 * 准确描述每一步需要的信息
 * */
public class HtmlStep extends AbstractExecuteStep {
    ...
    public EngineResult extcute(EngineContext engineContext) {
        ...
        if(!input.isEmpty()){
            String url = input.getString(getInputKey(Constants.HTML_URL,this));
            if(StringUtils.hasText(url)){
                ...
                try {
                    ...
                    String key = getInputKey(Constants.HTML_DATA,this);
                    output.put(key,docValue);
                } catch (IOException e) {
                    return EngineResult.Input_NULL;
                }
            }
        }else {
            return EngineResult.Input_NULL;
